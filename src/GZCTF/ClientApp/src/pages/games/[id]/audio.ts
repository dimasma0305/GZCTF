 ...
 * sfx surface + lifecycle (unlock / resume / setEnabled / close). All sounds are
 * synthesised on the fly (no audio files); the first-blood mp3 stays in the host.
 */
...
export function createSoundEngine(): SoundEngine {
  let AC: any = null, masterGain: any = null, reverb: any = null, enabled = true
  ...
  const on = () => enabled && audio() // every sfx gates on this

  return {
    unlock() { const ac = audio(); if (ac && ac.state === 'suspended') ac.resume(); if ('speechSynthesis' in window) { try { speechSynthesis.getVoices() } catch (e) {} } },
    resume() { try { if (AC && AC.state === 'suspended') AC.resume() } catch (e) {} },
    setEnabled(v: boolean) { enabled = v },
    isEnabled() { return enabled },
    close() { try { if (AC) AC.close() } catch (e) {} AC = null },
    sfxAttack() {
      if (!on()) return
