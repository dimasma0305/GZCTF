...
export const StatsPanel: FC = () => {
  ...
  const summaryCards = [
    {
      key: 'solves', icon: mdiPuzzle, color: 'teal',
      value: stats.totalSolves, label: t('account.stats.total_solves', 'Total Solves'),
    },
    {
      key: 'bloods', icon: mdiFire, color: 'orange',
      value: stats.totalFirstBloods, label: t('account.stats.first_bloods', 'First Bloods'),
    },
    {
      key: 'games', icon: mdiTrophy, color: 'blue',
      value: stats.gamesParticipated, label: t('account.stats.games', 'Games Played'),
    },
    {
      key: 'top', icon: mdiMedal, color: topCategory ? catColor(topCategory[0]) : 'gray',
                  ...
                  <Table.Tr key={g.gameId} className={classes.clickRow} onClick={() => navigate(`/games/${g.gameId}`)}>
                    <Table.Td>
                      <Group gap="xs" wrap="nowrap">
                        ...
                        />
                        <Text size="sm" lineClamp={1} fw={g.gameId === bestGameId ? 600 : 400}>{g.gameTitle}</Text>
                      </Group>
                    </Table.Td>
                    <Table.Td>
