...
Public submissions also pass a shape check (`ValidateSubmissionShape`) before reaching the review queue — they must include a non-empty `solver/` file, declare a flag source (`flags:` or `flagTemplate:`, unless the type uses the A&D engine), and (for container types) have a buildable Dockerfile in the conventional spot (`./src/Dockerfile` or `./Dockerfile`) when a build is implied by a local path or a `{{…}}` template image. The check only fails when a build is intended but no Dockerfile is found; an explicit registry image — or an empty image with no Dockerfile (which resolves to `None`, a non-build challenge) — passes through. Admin and binding imports skip this check (trusted).
